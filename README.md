...
| 버전 | 내용 |
|------|------|
| `5.3.20260615.2016` | **블록코딩 직전 안정본.** 인식및반응설정 표 칸 정렬(헤더·행을 같은 grid 컬럼으로 통일, mp3/tts/랜덤 값 칸 폭 일치) · 메인 창 **항상 위(📌)** 토글(기본 ON, 자식/설정 창 열릴 때 자동 해제·복귀) · 동작별 **🔁반복** 횟수 + **❓만약(조건)**(신뢰도·확률·같은 객체 수·낮/밤). 이후 블록코딩은 `block` 브랜치 v6.0~ 에서 진행 |

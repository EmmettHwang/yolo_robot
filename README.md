...
| `ultra` | ultralytics 통합본(YOLOv5/v8/v11). 인식·반응·학습 스튜디오·로봇 제어·영점 등 |
| `gradio` | 안정 완성본(v4.x). OpenCV DNN(ONNX) 추론 + 로컬 Gradio 학습 웹앱 |
| `block` | **(현재 작업 브랜치, v5.0~)** gradio 기반 + 인식및반응을 블록코딩으로 발전 |

### `block` 브랜치 목표 (v5.0~)

인식및반응설정을 **블록코딩/순서도**로 발전시킨다. 반응을 "작은 프로그램"으로:

- **동작 블록**: 모션 / 소리 / 전원 / 음성 대화
- **제어 블록**: 🔁 반복(N회), ❓ 만약(조건)→else, ⏱ 대기
- **조건(만약)**: 객체 수 / 신뢰도 / 확률(랜덤) / 배터리·시간대
- **트리거**: "○○이(가) 보이면"
- 발전 단계: **A) 표 에디터에 반복/만약** → B) 순서도(flowchart) 보기 → C) 블록 드래그 에디터

> gradio(v4.x)는 안정본으로 보존. block은 거기서 분기해 블록코딩만 더한다.

### (참고) `gradio` 브랜치 목표 (v4.0~)
...
| 버전 | 내용 |
|------|------|
| `5.1.20260615.1715` | A단계: 인식및반응 각 동작에 🔁반복(1~10회)+❓조건(신뢰도/확률/객체수/낮밤) 추가. 인식 시 조건 평가→건너뜀, 반복 실행, 로그 표시 |
| `5.0.20260615.1706` | **메이저 업 + `block` 브랜치 시작** — gradio(v4.x)에서 분기. 인식및반응을 반복/만약(블록코딩)·순서도로 발전 예정 |
| `4.28.20260615.0223` | 인식및반응설정 변경을 자율활동 중에도 실시간 반영(설정 파일 mtime 1초 감지→자동 재로드) |
| `4.27.20260615.0214` | 자율활동 탭 복귀 시 화면 멈춤 해결(on_show 렌더 재가동)+매핑 자동 반영, 정지 안내 표시, 화면 밑 반응 로그 박스(시퀀스 단계 표시) |
| `4.26.20260615.0208` | TTS 안정화: Windows SAPI(PowerShell, UTF-8) 우선·pyttsx3 폴백 — 스레드 반복 호출 시 음성 안 나오던 문제 해결 |
| `4.25.20260615.0201` | 음성 대화 모션(1003) 추가 — 마이크(Google STT)→로컬 Ollama LLM→스피커(pyttsx3). 인식및반응설정에 음성대화 모델 선택(Ollama) |
| `4.24.20260615.0157` | 자율활동 인식 초기화 버튼 추가 · 반응의 전원 켜기/끄기 시퀀스에 효과음+명령 간격(0.3s) 보강 |
| `4.23.20260615.0148` | 인식및반응설정: 모션 목록에 전원 켜기/끄기(1001/1002, 동일 시퀀스) 추가 + 서브 동작 순서 ▲/▼ 이동 |
| `4.22.20260615.0133` | 전원 끄기 효과음을 전원이 실제 꺼진 뒤(앉기→7초→OFF 완료 후)에 재생 |
| `4.21.20260615.0130` | 전원 켜기 일어서기 신뢰성↑: 전원 ON 2회+토크 안정 0.6초+일어서기 2회(첫 패킷 씹힘 대비), 끄기도 앉기/OFF 2회 |
| `4.20.20260615.0126` | 전원 켜기 일어서기 누락 수정: 로봇제어 시퀀스(ON→0.3초→일어서기) 그대로 + 새 BT 연결 직후 0.6초 안정화(첫 패킷 누락 방지) |
| `4.19.20260615.0122` | 전원 켜기: ON 후 0.5초 대기 후 일어서기(안정) + 전원 ON/OFF 효과음(FX_POWER_ON/OFF) |
| `4.18.20260615.0119` | 로봇장치설정 전원 켜기/끄기를 로봇제어 safe_power와 동일 시퀀스로(켜기=전원ON→일어서기61, 끄기=앉기60→7초→전원OFF) |
| `4.17.20260615.0112` | 모션 드롭다운 옆 'LED 중 이 동작 함께' 체크박스 — 체크 시 LED 사이클마다 선택 모션 함께 실행 |
| `4.16.20260615.0110` | 로봇장치설정에 전원 켜기/끄기 버튼 추가(끄기=Safe Sit 후 토크 OFF, LED 중엔 예약 전송) |
| `4.15.20260615.0059` | 고개 회전 ±30도로 / LED 진행 중 동작 테스트 누르면 같은 연결로 모션 끼워 전송(예약→LED 스레드가 전송) |
| `4.14.20260615.0054` | LED 테스트 중 모션 병행: 순차 18번까지 켜진 뒤 고개(ID18) 좌우 15도 흔들고 중앙 복귀 → 깜빡 |
| `4.13.20260615.0048` | 순차 점등: 페이드 아웃 제거(켜진 채 누적), 모터당 페이드 인 200ms, 전체 점등 후 200ms 간격 3회 깜빡 → 무지개 |
| `4.12.20260615.0041` | 순차 점등을 모터 ID 순서(1,3,5..16,17,18)로 한 모터씩 페이드 인/아웃(≈500ms)·사이클마다 색 변경 / 동작 테스트 모션 기본값 1번 |
| `4.11.20260615.0033` | LED 테스트 개편: 순차 점등=그룹별 단일색(켜고 0.2초→끄고 다음), 무지개=느린 페이드 인/아웃+천천히 회전 |
| `4.10.20260609.2125` | 인공지능학습 탭 '주소 저장' 버튼을 '학습 웹앱 열기'와 같은 보라색으로 통일(시인성) |

...
def speak_blocking(text: str) -> None:
    """텍스트를 음성으로 출력(블로킹). Windows는 SAPI(PowerShell)로 안정 재생,
    실패 시 pyttsx3 폴백. 한 번에 하나씩만(락)."""
    text = (text or "").strip()
    if not text:
        return
    with _tts_lock:
        # 1) Windows 내장 SAPI — 스레드/반복 호출에도 안정적
        if sys.platform == "win32":
            ...
            try:
                ...
                ps = ("Add-Type -AssemblyName System.Speech; "
                      ...
                      "SpeechSynthesizer; "
                      "$t = Get-Content -Raw -Encoding UTF8 '%s'; "
                      "$s.Speak($t)" % path.replace("'", "''"))
                ...
                print(f"[sound] SAPI TTS 실패, pyttsx3로 폴백: {e}")
            finally:
                if path:
                    try:
                        os.remove(path)
        ...
        if _HAS_TTS:
            try:
                ...
                print(f"[sound] pyttsx3 TTS 실패: {e}")
